...
global_heart=[]
thresh_board_value=155

save_image_list =[]
output_list = []
frame_num_list = []
output1 = "datasets/13"  # 指定输出文件夹路径
save_radiu=30
frame_num = 0
def save_frames(image, output_folder, frame_count):
    """
    保存图像到指定文件夹。
    ...
    if circles is not None:
        ...
        for circle in circles[0, :]:
            ...
            if flag ==1:
                ...
                # save_x1 = max(0, center_x - 30)
                # save_y1 = max(0, center_y - 30)
                # save_x2 = min(image.shape[1], center_x + 30)
                # save_y2 = min(image.shape[0], center_y + 30)
                save_x1 = max(0, center_x - save_radiu)
                save_y1 = max(0, center_y - save_radiu)
                save_x2 = min(image.shape[1], center_x + save_radiu)
                save_y2 = min(image.shape[0], center_y + save_radiu)
                save_image = image[save_y1:save_y2, save_x1:save_x2]
                save_image2 = image[save_y1-10:save_y2+10, save_x1-10:save_x2+10]

                save_image = cv2.resize(save_image,(60,60))
                save_image2 = cv2.resize(save_image2,(60,60))

                save_image_list.append(save_image)
                save_image_list.append(save_image2)
                output_list.append(output1)
                output_list.append(output1)
                frame_num_list.append(int(time.time()*100000)+frame_num)
                frame_num_list.append(int(time.time()*400000)+frame_num)
                #save_frames(save_image, output1, )
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            a = cv2.waitKey(1)
            if a & a == ord('s'): #保存图片
                for index,save_image in enumerate(save_image_list):
                    save_frames(save_image, output_list[index], frame_num_list[index])
            if a & a == ord('q'):
                break
        except:
            pass
